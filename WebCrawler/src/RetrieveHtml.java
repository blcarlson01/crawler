...
public class RetrieveHtml {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		try {
			...
			// Science
				String page = "http://slashdot.org/archive.pl?op=sections&keyword=developers&year=2010&page="+x;
				doc = Jsoup.connect(page).timeout(20*1000).get();
				
				// Save Original File
				String orgFile = "/Users/blcarlson/Documents/Grad Program/CS410/hw2/ask.slashdot.org/org/"+uniqueId+".html";
				...
				for (Element link : links) {
					...
					if (linkHref.contains("//developers.slashdot.org/story")
							...
							&& !foundUrls.contains(linkHref.replace("//", "http://"))) {
						...
						if(!f.exists()) { 
						String articleLink = linkHref.replace("//", "http://");
						...
						doc = Jsoup.connect(articleLink).timeout(20*1000).get();
						...
						// Parse the webpage and save required text to text file
						CreateTextFile(doc, articleId, articleLink, linkText);

						System.out.println("Successfully wrote: " + htmlFile);
						System.out.println("-----------------------");
						// Wait to not anger the Internet gods
						TimeUnit.SECONDS.sleep(8);
						}
